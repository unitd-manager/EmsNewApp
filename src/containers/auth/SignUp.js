...
export default function SignUp() {
  ...
  const [conpasswordInputStyle, setConPasswordInputStyle] =
  ...
  const SendEmail = () => {
    ...
    api.post('/commonApi/sendUseremailSignUp', {to, subject}).then(response => {
      if (response.status === 200) {
        Alert.alert('You have successfully registered');
        setTimeout(() => {
          navigation.navigate(StackNav.Login);
  ...
  const PhoneIcon = () => {
    return (
      <FontAwesome name="phone" size={moderateScale(20)} color={'black'} />
    );
  };
